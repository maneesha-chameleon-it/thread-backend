...
class StyleRoute implements Routes {
  ...
  private initializeRoutes() {
    ...
    this.router.post(`${this.path}`, validationMiddleware(CreateStyleDto, 'body'), requestJson(this.styleController.createStyle, 'CREATE_Style'));
    this.router.put(`${this.path}/:id(\\d+)`, validationMiddleware(CreateStyleDto, 'body', true), requestJson(this.styleController.updateStyle, 'UPDATE_Style'));
    this.router.delete(`${this.path}/:id(\\d+)`, requestJson(this.styleController.deleteStyle,'DELETE_Style'));
  }
}
